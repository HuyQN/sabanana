...
export default class EditPost extends React.Component {
  constructor (props) {
    super(props)
    this.postID = this.props.match.params.query;
    this.state = this.props
    this.extra = []
  }

  // information for when page starts up
  /*startingUp(){
    document.getElementById("titleArea").value =this.postData.name;
    document.getElementById("descArea").value=this.postData.description;
    this.postData.tags.map(function(item){
      if(blist.lastIndexOf(item) == -1){
        this.extra[this.extra.length] = item;
      }
    })
